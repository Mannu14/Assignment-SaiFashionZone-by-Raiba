...
const Complaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const response = await fetch(`${window.location.origin}/get-complaints`);
  ...
  const handleStatusChange = async (complaintId, newStatus) => {
    try {
      const response = await fetch(`${window.location.origin}/update-complaint-status`, {
        method: 'PUT',
        headers: {
          ...
          <div className="mb-6 flex justify-between items-center">
            <div className="relative w-full max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-300 bg-white shadow-sm transition-all duration-300"
                value={searchQuery}
                  ...
                  <tr>
                    {[
                      ...
                      <th 
                        ...
                        className={`px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider ${key ? 'cursor-pointer hover:bg-gray-200 transition-colors' : ''}`}
                      >
                        <div className="flex items-center">
                          {label}
                          {key && <ArrowUpDown size={14} className="ml-2 text-gray-400" />}
                        </div>
                      </th>
                    ))}
                  </tr>
                            ...
                            <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                              <ul className="py-1">
                                <li 
                                  ...
                                  className="px-4 py-2 hover:bg-gray-50 cursor-pointer text-sm text-gray-700 hover:text-gray-600"
                                >
                                  Pending
                                </li>
                                <li 
