...
const Product = () => {
  ...
  const handleSave = async (productId) => {
    try {
      const response = await fetch(`${window.location.origin}/instock-update`, {
        method: 'POST',
        headers: {
  ...
  const handleVisibilityChange = async (productId, newVisibility) => {
    try {
      const response = await fetch(`${window.location.origin}update-visibility`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
          ...
          <div className="relative">
            <div className={`flex items-center ${isSearchExpanded ? 'w-full md:w-64' : 'w-10 md:w-64'} transition-all duration-300`}>
              ...
              <input
                ...
                className={`pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-300 ${
