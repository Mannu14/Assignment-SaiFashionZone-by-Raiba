...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`${window.location.origin}/product/${productId}`);
      ...
      <div className="min-h-screen bg-gradient-to-b from-pink-50 to-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div 
            ...
            className="bg-white rounded-1xl shadow-2xl overflow-hidden border border-pink-100"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
              {/* Product Image Section */}
              <div className="p-8 bg-pink-100 flex items-center justify-center">
                <motion.div 
                ...
                >
                  <img
                    src={product.img}
                    alt={product.name}
                    className="absolute inset-0 w-full h-full object-contain rounded-1xl shadow-lg"
                  />
                </motion.div>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  {/* Add to Cart Button */}
                    <motion.button 
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      className={`w-full py-4 rounded-full hover:shadow-xl transition duration-300 flex items-center justify-center space-x-3 ${
                        stockStatus?.stock === 0 
