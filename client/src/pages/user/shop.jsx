...
const Shop = ({ category }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${window.location.origin}/get-product`, {
          method: 'GET',
          credentials: 'include',
        });
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-b from-red-50 to-red-100 min-h-screen">
        <Navbar />
        ...
        >
          <div className="max-w-4xl mx-auto">
            <h2 className="text-5xl font-extrabold text-red-800 mb-4 tracking-tight">Discover Our Curated Collections
            </h2>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          <h3 className="text-3xl font-bold mb-8 text-red-900 text-center">Explore Our Categories</h3>
            ...
            {categories.map((category, index) => (
              <motion.div
                ...
                className={`bg-white rounded-xl shadow-lg overflow-hidden transform transition-all duration-300 
                  ...
                  ${selectedCategory === category.name ? 'border-4 border-red-500' : ''}`}
              ...
              >
                ...
                <div className="p-5 text-center">
                  <h4 className="text-2xl font-bold text-red-800 mb-2">{category.name}</h4>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0">
            <div className="flex items-center space-x-4">
              <button
                ...
                className="flex items-center bg-red-100 text-red-800 px-4 py-2 rounded-lg hover:bg-red-200 transition"
              >
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                <FaSort className="mr-2 text-red-800" />
                <select
                  className="border-red-300 border px-3 py-2 rounded-lg focus:ring-2 focus:ring-red-500 transition"
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-red-500 text-white' : 'bg-red-100 text-red-800'} hover:bg-red-500 hover:text-white transition`}
                >
                  <FaThLarge />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-red-500 text-white' : 'bg-red-100 text-red-800'} hover:bg-red-500 hover:text-white transition`}
                >
                  <FaList />
                </button>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <AnimatePresence>
            ...
            >
              {filteredProducts.slice(0, loadMore).map((product) => (
                <motion.div
                  key={product._id}
                  className={`bg-white shadow-lg rounded-1xl overflow-hidden relative 
                ...
                >
                  ...
                  >
                    ...
                    >
                      <button
                        className="w-full bg-red-50 text-red-600 py-2 rounded-md 
                        hover:bg-red-100 transition-colors" onClick={e => { addPostToRecentlyViewed(product._id) }}
                      >
                        View Details
                      </button>
                    </Link>
          ...
          <div className="text-center mt-12">
            {loadMore < filteredProducts.length ? (
              <button
                className="bg-red-600 text-white px-8 py-3 rounded-full hover:bg-red-700 transition-colors duration-300 shadow-lg hover:shadow-xl"
        ...
        <footer className="bg-white py-16 text-black border-t border-red-200">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 px-6">
            <div className="flex flex-col items-center md:items-start">
              <h4 className="text-3xl font-extrabold text-red-800 mb-4">SaiFashionZone by Raiba</h4>
              <p className="text-gray-600 mb-4 text-center md:text-left">
                Your one-stop destination for thoughtful and unique gifts.
              </p>
              <div className="flex space-x-6 text-3xl mt-4">
                <FaFacebook className="text-red-600 hover:text-red-800 transition cursor-pointer" />
                <FaInstagram className="text-red-600 hover:text-red-800 transition cursor-pointer" />
                <FaTwitter className="text-red-600 hover:text-red-800 transition cursor-pointer" />
              </div>
            </div>
            <div className="text-center md:text-right">
              <h5 className="text-2xl font-bold text-red-800 mb-4">Contact Us</h5>
              <p className="text-gray-600">
                National Games Housing Complex, Ranchi Jharkhand
                <br />
                Email: support@saifashionzonebyraiba.com
                <br />
                Phone: +1 (875) 563-2536
              </p>
            </div>
          </div>
        </footer>
