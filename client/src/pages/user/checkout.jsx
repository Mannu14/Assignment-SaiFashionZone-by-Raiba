...
const Checkout = () => {
  ...
  const fetchCartItems = async () => {
    ...
    try {
      ...
      const productPromises = Object.values(groupedItems).map(async (item) => {
        const productResponse = await fetch(`${window.location.origin}/product/${item.productId}`);
  ...
  const handlePlaceOrder = async () => {
    ...
    if (saveAddress) {
      try {
        await fetch(`${window.location.origin}/update-address`, {
          method: 'POST',
          headers: {
    ...
    try {
      const response = await fetch(`${window.location.origin}place-order`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
