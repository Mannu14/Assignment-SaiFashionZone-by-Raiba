...
const CartItems = () => {
    ...
    const fetchCartItems = async () => {
      ...
      try {
        ...
        const productPromises = Object.values(groupedItems).map(async (item) => {
          const productResponse = await fetch(`${window.location.origin}/${item.productId}`);
  ...
  const handleRemoveItem = async (itemId) => {
    try {
      const userId = sessionStorage.getItem('userId');
      const response = await fetch(`${window.location.origin}/delete-items`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64 bg-gray-50">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-gray-600"></div>
      </div>
    );
  }
