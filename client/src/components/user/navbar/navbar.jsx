...
const ProfessionalNavbar = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      if (!userId) return;
      const cartResponse = await fetch(`${window.location.origin}/cart/${userId}`);
  ...
  useEffect(() => {
    ...
    const fetchUserName = async () => {
      ...
      if (userId) {
        try {
          const response = await fetch(`${window.location.origin}/auth/user/${userId}`);
  ...
  return (
    <nav className={`top-0 left-0 w-full z-50 transition-all duration-300 mb-auto ${
      ...
      <div 
        className={`bg-gray-600 text-white py-2 text-center text-xs transition-all duration-300 ${
      ...
      <div className="bg-white border-b">
        <div className="max-w-[1200px] mx-auto px-4 lg:px-0">
          <div className="h-[70px] flex items-center justify-between">
            ...
            >
              <span className="font-['Bodoni_MT'] font-bold text-2xl text-gray-600">SaiFashionZone by Raiba</span>
            </Link>
              ...
              {[
                ...
                <Link
                  ...
                  className={`
                    ...
                    ${isActive(path) ? 'text-gray-600' : 'text-gray-800'}
                    hover:text-gray-600
                  `}
                >
                  {name}
                  <span 
                    className={`
                      absolute bottom-[-4px] left-0 w-0 h-0.5 bg-gray-600 
            ...
            <div className="flex items-center space-x-6">
              ...
              <Link 
                to="/cart" 
                className="relative text-gray-800 hover:text-gray-600 transition flex items-center"
              >
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          >
            ...
            <div className="p-6">
              <button 
                onClick={toggleMenu} 
                className="absolute top-5 right-5 text-gray-600 hover:text-gray-600"
              >
              ...
              <div className="mt-12 space-y-4">
                {userId ? (
                  <Link
                    to="/logout"
                    onClick={handleLogout}
                    className="py-2.5 text-lg font-medium text-gray-800 hover:text-gray-600 transition flex items-center"
                  >
